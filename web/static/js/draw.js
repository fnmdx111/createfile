...
(function() {
  var container, fc_container, fill_ct_text, fire_post, _data, _hide_loading, _idx_table, _show_loading,
    __slice = [].slice;
  ...
  fill_ct_text = function(method) {
    var d, p, segments, t, texts, tf, _d;
    if (method === 'time') {
    ...
    texts = ((function() {
      ...
      for (_i = 0, _len = _d.length; _i < _len; _i++) {
        d = _d[_i];
        t = d[0], segments = 2 <= d.length ? __slice.call(d, 1) : [];
        if (segments.length === 0) {
          continue;
        }
        tf = moment(Math.floor(t)).format('YYYY/MM/DD HH:mm:ss');
        p = _idx_table[t.toString() + '.0'][segments[0][0].toString()];
        _results.push("<li>" + tf + " " + p + ": " + segments + "</li>");
      }
      return _results;
    })()).join('');
    return $('#ct-text').html("<ul>" + texts + "</ul>");
  };
